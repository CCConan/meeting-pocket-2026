...
document.querySelector('#strategy-index').innerHTML=content.strategy.map((s,i)=>`<a class="question-link strategy-link" href="#strategy-${i+1}" aria-label="策略 ${i+1}：${escapeHTML(s.title)}"><span class="number">${String(i+1).padStart(2,'0')}</span><span class="label">${escapeHTML(s.title.split(' · ')[1]||s.title)}</span></a>`).join('');
document.querySelector('#strategy-text').innerHTML=content.strategy.map((s,i)=>`<article class="strategy-card" id="strategy-${i+1}" tabindex="-1"><h3>${escapeHTML(s.title)}</h3>${pair([s.zh,s.en])}<a class="back-index" href="#strategy">返回 8 點索引 ↑</a></article>`).join('');
...
function jump(hash){const target=document.getElementById(hash.slice(1));if(!target)return;target.scrollIntoView({behavior:'instant',block:'start'});if(target.matches('.answer,.strategy-card'))target.focus({preventScroll:true});document.querySelectorAll('.question-link,.bottom-nav a').forEach(a=>a.setAttribute('aria-current',String(a.getAttribute('href')===hash)));}
